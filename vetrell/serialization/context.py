...
# Try to import dill, fall back to pickle if not available
try:
    import dill
except ImportError:
    dill = pickle
...
# =============================================================================
# SIGNAL TYPES AND PRIORITIES (Standalone definitions)
# =============================================================================

class SignalType(Enum):
    """Types of signals that can be transmitted."""
    CUSTOM = auto()
    CONTEXT_UPDATE = auto()
    STATE_CHANGE = auto()
    ERROR = auto()
...
class Observer(ABC):
    ...
    async def on_signal(self, signal: 'SignalPayload') -> bool:
        """Handle incoming signal. Return True if handled."""
        pass


@dataclass
class SignalPayload:
    """Standard signal payload for transmission."""
...
class Signal:
    ...
    # Serialized context in metadata
    ...
    def attach_serialized_context(self, context: 'SerializableExecutionContextWithPortability'):
        """Attach serialized context to signal metadata."""
        # Serialize the complete context
        serialized = context.serialize_for_transmission()
        ...
        self.context_metadata = SerializedContextMetadata(
            ...
            dependencies=context.get_dependency_bundle()
        )
    ...
    def extract_serialized_context(self) -> Optional['SerializableExecutionContextWithPortability']:
        ...
        try:
            ...
            # Reconstruct context
            context = SerializableExecutionContextWithPortability(
                context_id=serialized_data.get('context_id')
            )
            return context
...
class ContextAwareSignalObserver(Observer):
    ...
    def __init__(self, signal_filters: List[SignalType] = None):
        ...
        self.extracted_contexts: Dict[str, 'SerializableExecutionContextWithPortability'] = {}
    ...
    async def on_context_extracted(self, signal: 'Signal', context: 'SerializableExecutionContextWithPortability'):
